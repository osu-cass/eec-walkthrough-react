					...
					if (results.ok) {
						...
						for (let i = 0; i < obj.notifications.length; i++) {
							if (obj.notifications[i].type === 6) {
								...
								obj.notifications[
									i
								].address = `/edit-quiz/${obj.notifications[i].requestId}`
							} else {
								obj.notifications[
									i
								].address = `/publish-requests/${obj.notifications[i].requestId}`
							}
						}
				...
				{notifications.length ? (
					<Fragment>
						{notifications.map((item, index) => (
							...
							>
								<div className="dropdown-item note-item">{item.text}</div>
							</Link>
						))}
					</Fragment>
				) : (
					<Link to={`.`} onClick={event => event.preventDefault()}>
						<div className="dropdown-item note-item">No new notifications</div>
					</Link>
				)}
			</div>
		</div>
